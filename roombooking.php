                        ...
                        <tbody>
                            ...
                            if ($result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    echo "<td>
                                                <a href='#' onclick='openViewModal(" . htmlspecialchars($row['room_id']) . ")'><i class='bx bx-show'></i></a> |
                                                <a href='#' onclick='openModal(" . htmlspecialchars($row['room_id']) . ")'><i class='bx bx-edit'></i></a> |
                                                <a href='#' onclick='openCheckoutModal(" . htmlspecialchars($row['room_id']) . ")'><i class='bx bx-log-out'></i></a> |
                                                <a href='roombooking.php?id=" . htmlspecialchars($row['room_id']) . "&del=delete'
                                                onClick='return confirm('Are you sure you want to delete?')'><i class='bx bx-trash'></i></a>
                                            </td>
                                        </tr>
                                            </td>";
                                    echo "</tr>";
    ...
    <div id="checkoutModal" class="modal">
        <div class="modal-content">
            ...
            <form id="checkoutForm">
                <input type="hidden" id="checkoutRoomId" name="roomId">
                <label for="checkoutStatus">Status:</label>
                <select id="checkoutStatus" name="status">
...
document.getElementById("checkoutForm").onsubmit = function(e) {
    e.preventDefault();
    var roomId = document.getElementById("checkoutRoomId").value;
    var status = document.getElementById("checkoutStatus").value;
    ...
    fetch('update_checkout.php', {
        ...
        body: 'roomId=' + encodeURIComponent(roomId) + '&status=' + encodeURIComponent(status)
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
